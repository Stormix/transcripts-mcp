...
Currently configured project: `transcripts-mcp` (target: `..`).
...
original init command. Checkpoints in `data/transcripts-mcp/setup/setup-state.json`
...
`--project-id transcripts-mcp` (or whichever id you want) explicitly.

`scan` is free (regex only). `process` is the AI stage (≈$0.30/file
on Opus by default). Run state goes to `data/transcripts-mcp/`.
