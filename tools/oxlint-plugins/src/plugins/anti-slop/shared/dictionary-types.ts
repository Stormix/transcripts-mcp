...
import { typeReferenceName } from "./ast.ts";
...
export const classifyUnsafeDictionaryValue = (
  ...
  environment: TypeEnvironment,
): UnsafeDictionaryValue | null => unsafeDirectValue(valueType, environment, new Map(), new Set());
...
const resolvesToDictionary = (
  ...
  substitutions: TypeAliasEnvironment,
  resolvingAliases: ReadonlySet<string>,
): boolean => dictionaryValueTypes(type, environment, substitutions, resolvingAliases).length > 0;
...
export const classifyWideningTarget = (
  ...
  if (unwrapped.type === "TSTypeLiteral") {
    ...
    return unwrapped.members.length > 0 ? "anonymous object" : null;
  }
