...
import { collectTopLevelTypeAliases, isGenericAlias, referencedAliasName } from "../shared/ast.ts";
...
export const noUnknownTypeAliasesRule = defineRule({
  ...
  create(context) {
    ...
    const resolvesToUnknown = (type: ESTree.TSType, visited = new Set<string>()): boolean => {
      ...
      if (alias === undefined || isGenericAlias(alias)) return false;
    ...
    return {
      Program(node) {
        for (const [name, alias] of collectTopLevelTypeAliases(node)) {
